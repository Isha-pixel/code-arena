...
//   Select, Button, HStack, useToast, Tag, Wrap, WrapItem
...
//   const [tagsInput, setTagsInput] = useState('array, two pointers');
...
//     if (!title.trim()) {
//       toast({ status: 'warning', description: 'Title is required' });
//       return;
//     }
//     setSaving(true);
//     try {
//       const payload = { title, difficulty, tags, statement };
//       const { data } = await api.post('/problems/', payload);
//       toast({ status: 'success', description: 'Problem created' });
//       // go to detail if your route uses /problems/:id
//       nav(`/problems/${data.id}`);
//     } catch (e: any) {
//       toast({ status: 'error', description: e?.response?.data ?? 'Failed to create problem' });
//     } finally {
//       setSaving(false);
//     }
//   };
...
//           <Input value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g., Closest Refuge" />
//         </FormControl>
...
//             rows={8}
//             placeholder={`Describe the problem, input/output format, constraints, samples...`}
//           />
//         </FormControl>

//         <HStack>
//           <Button colorScheme="teal" isLoading={saving} onClick={handleSubmit}>Create</Button>
//         </HStack>
//       </MBox>
//     </Box>
//   );
